...
\subsection{Some variants of Hierarchical Gaussian descriptors}
Hierarchical gaussian descriptor characterizes texture, color and $y$ coordinate of an image. The importance of those three characteristics is ordered as color values > gradient components > $y$ coordinate. A rank-1, rank-5, rank-10, and rank-20 comparison of three variants on VIPeR dataset are listed below. We can conclude that color values have the most influence on Re-ID accuracy. Based on this conclusion, in GOG$_\text{fusion}$ variants test the color values will remain unchanged and $y$ coordinate and M components will be replaced or removed to study performance. One GOG$_\text{fusion}$ variant is to replace the M components in $\bm{f}_i$ with LBP. 
...
Besides, gaussian mixture model based descriptor is also tested.
