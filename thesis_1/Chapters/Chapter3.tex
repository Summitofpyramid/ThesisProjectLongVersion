...
Hierarchical gaussian descriptor has a few advantages compared with other descriptors. Firstly, it has a full consideration of color and texture information. The color information is adopted by adding color values of different color space to $\bm{f}_i$. Textural information is also adopted by those four $\bm{M}$ components in $\bm{f}_i$. Secondly, by Riemannian manifold based SPD mapping, it provides a solution to summarize many multi-variate gaussian functions. The parameters $\bm{\mu}$ and $\bm{\Sigma}$ can be fused into a SPD matrix. With later vectorization this SPD matrix is transformed to a vector. In this mapping process correlation of different components in components of $\bm{f}_i$ are fully taken into account, which leads to a dimension increasing from basic pixel feature vector $\bm{f}_i$ to patch gaussian vector $\bm{p}_j$. Thirdly, by a two-level gaussian model, this model guarantees to extract the overall statistical information of an image while not putting too much emphasis on local details. This helps to overcome the viewpoint variation. 

It is intractable to directly learn a Mahanalobis distance matrix from concatenated Hierarchical Gaussian descriptor. In next Chapter the KLFDA is applied to reduce the dimensionality of extracted descriptor from 27622 to $n-1$, where $n$ is the number of different classes. With this supervised nonlinear dimensionality reduction, discriminative information among different classes are preserved. At last a Mahanalobis matrix is learned on the dimension reduced descriptors by gradient descent method.
